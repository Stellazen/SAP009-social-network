...
function verificaUsuarioLogado(verificando) {
  onAuthStateChanged(auth, verificando);
}
const createUser = async (email, senha, displayName) => {
  const authentication = getAuth(app$1);
...
const valuesLogin = (email, senha) => signInWithEmailAndPassword(auth, email, senha);
const provider = new GoogleAuthProvider();
const googleLogin = () => signInWithPopup(auth, provider);
const sairPerfil = () => signOut(auth);
const login = () => {
  ...
  const template = `
    ...
    <section class="fundo-form">
          <section class="texto-entrada">
            <p>A sua rede social para compartilhar dicas e receitas de dar \xE1gua na boca</p>
  ...
  const fazerLogin = () => {
    btn.addEventListener("click", () => {
      valuesLogin(email.value, senha.value).then(() => {
        window.location.href = "#feed";
      }).catch(() => {
        txtError.setAttribute("style", "display: block");
        txtError.innerHTML = "Usu\xE1rio ou senha incorretos";
      });
    });
  };
  fazerLogin();
  btnGoogle.addEventListener("click", () => googleLogin().then(() => {
...
const db = wh(app);
async function newPost(dataPostagem, id2, post, username) {
  const docRef = await uf(sh(db, "post"), {
    date: dataPostagem,
    idUser: id2,
    textArea: post,
    userName: username,
  ...
  const postUnic = await Jl(docRef);
  return postUnic;
}
function postsNaTela(limparTela, testando) {
...
const getPost = async (postId) => {
  const ref = rh(db, "post", postId);
  const result = await Jl(ref);
  return result.data();
};
const likePost = async (postId, idUser) => {
...
async function deletarPost(postId) {
  await of(rh(db, "post", postId));
}
const editPost = (postId, textArea) => rf(rh(db, "post", postId), {
  textArea
});
const feed = () => {
  const container = document.createElement("div");
  ...
  <p class='username' id='username' >Ol\xE1, ${auth.currentUser.displayName}</p>
  </div>
    <section class="post">
  ...
  btnSair.addEventListener("click", () => sairPerfil().then(() => {
  ...
  postagem.addEventListener("click", async (event) => {
    ...
    if (element.dataset.likeId) {
      ...
      if (likes.liked === true) {
        imgLike.setAttribute("src", "../img/panela-preenchida.png");
      } else {
        imgLike.setAttribute("src", "../img/panela.png");
      }
      ...
      if (window.confirm("Tem certeza que gostaria de deletar essa postagem?")) {
        deletarPost(element.dataset.excluirId).then(() => {
          alert("deletado com sucesso");
        }).catch(() => {
          alert("N\xE3o foi poss\xEDvel deletar sua postagem. Tente novamente.");
        });
      }
    }
  });
...
const cadastro = () => {
            ...
            <div class="coluna-2">
                <div class="cadastro">
                    ...
                    <section class="container-cadastro">
                            <section class="imagem-logo">
                                <img class="logo-escrita-escura" src="./img/logo-escrita-escura.png" alt="Logo da Br\xFAlle com a escrita escura">
                            </section>
  ...
  criarCadastro.addEventListener("click", (e) => {
    ...
    createUser(email, senha, nome).then(() => {
...
const devKeila = "/assets/devKeila.650600f2.png";
const devStella = "/assets/devStella.849a6a11.png";
const iconeGitHub = "/assets/iconeGitHub.4a26ecb7.png";
const sobre = () => {
  const container = document.createElement("div");
  const template = `
  <div>  
        ...
        <section class='main-sobre'>
                <div>
                    <div class='description'>
                        <p class='texto-sobre'>A Br\xFAlle \xE9 uma rede social criada para os amantes da Gastronomia se conectarem e compartilharem receitas, dicas de restaurantes e tudo que envolve o mundo gastron\xF4minco.</p>
                    </div>
                    <p class='devs-sobre'>Conhe\xE7a as Desenvolvedoras:</p>
                </div>
